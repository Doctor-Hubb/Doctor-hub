...
-- لیست 13 اسم bus1 تا bus13 برای پارت‌ها
local TARGET_NAMES = {
...
local function gatherTargets()
    ...
    for _, targetName in ipairs(TARGET_NAMES) do
        for _, inst in ipairs(Workspace:GetDescendants()) do
            if inst:IsA("BasePart") and inst.Name == targetName then
                table.insert(targets, inst)
                break -- فقط اولین پارت با این نام را پیدا کن
            end
        end
    end
end
...
Workspace.DescendantAdded:Connect(function(desc)
    if running and desc:IsA("BasePart") then
        for _, targetName in ipairs(TARGET_NAMES) do
            if desc.Name == targetName then
                gatherTargets() -- لیست را دوباره جمع آوری کن
                break
            end
        end
    end
...
local function runAutoPickup()
    while running do
        ...
        -- به ترتیب bus1 تا bus13 حرکت می‌کند (همان ترتیب آرایه)
        for i = 1, #targets do
            if not running then break end
            ...
            if teleportTo(part) then
                -- فقط تلپورت می‌کند و هیچ دکمه‌ای فشار نمی‌دهد
                if part and part.Parent then
                    task.wait(TIME_BETWEEN)
                else
                    break
...
-- Toggle
local Toggle = FarmTab:CreateToggle({
    Name = "Auto Farm bus(Aval Savar Otobos Shavid)",
    CurrentValue = false,
    Flag = "Auto Farm Tras",
    Callback = function(Value)
        running = Value
